#!/usr/bin/env python
"""
Retrain Left Eye Openness Model using a pre-existing model for relabeling.
...
def load_data_from_db(db_path: str):
    """
    Load left eye images and openness labels from the SQLite database.
    Only rows where leftEyeFrame is non-empty and type is 'openness' are used.
    """
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT leftEyeFrame, openness
        FROM training_data
        WHERE leftEyeFrame != '' AND type == 'openness'
        ORDER BY RANDOM()
        LIMIT 35000
    """)
    ...
    images = []
    labels = []
    for left_frame, openness in rows:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Retrain Left Eye Openness Model using a pre-existing model for relabeling"
    )
    ...
    new_model_path = os.path.join(output_dir, "left_openness_gen2.h5")
